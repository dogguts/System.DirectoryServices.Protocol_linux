...
namespace System.DirectoryServices.Protocols {
    ...
    internal delegate string PtrToString(IntPtr ptr);
    internal delegate IntPtr StringToHGlobal(string s);
    ...
    public class LdapConnection : DirectoryConnection, IDisposable {
        ...
        internal PtrToString PtrToString = Marshal.PtrToStringUTF8; //TODO: Windows:PtrToStringUni , Linux: PtrToStringUTF8
        internal StringToHGlobal StringToHGlobal = Wldap32.StringToHGlobalUTF8;//TODO: Windows:Marshal.StringToHGlobalUni, Linux: Wldap32.StringToHGlobalUTF8
        ...
        public void Abort(IAsyncResult asyncResult) {
            ...
            if (!(asyncResult is LdapAsyncResult)) {
                throw new ArgumentException(String.Format(SR.NotReturnedAsyncResult, nameof(asyncResult)));
            }
                ...
                if (request is DeleteRequest) {
                    ...
                    if (attributeCount != 0) {
                        ...
                        for (i = 0; i < attributeCount; i++) {
                            IntPtr controlPtr = StringToHGlobal(searchRequest.Attributes[i]);
                ...
                if (controlList.Count != 0) {
                    ...
                    for (int i = 0; i < count; i++) {
                        managedControls[i] = new LdapControl() {
                            ...
                            ldctl_oid = StringToHGlobal(((DirectoryControl)controlList[i]).Type),
        ...
        internal LdapMod[] BuildAttributes(CollectionBase directoryAttributes, ArrayList ptrToFree) {
            ...
            if (directoryAttributes != null && directoryAttributes.Count != 0) {
                ...
                for (int i = 0; i < directoryAttributes.Count; i++) {
                    ...
                    attributes[i].attribute =StringToHGlobal(modAttribute.Name);
        ...
        internal unsafe int ConstructParsedResult(IntPtr ldapResult, ref int serverError, ref string responseDn, ref string responseMessage, ref Uri[] responseReferral, ref DirectoryControl[] responseControl) {
            ...
            try {
                ...
                if (resultError == 0) {
                    // Parse the dn.
                    responseDn = PtrToString(dn);

                    // Parse the message.
                    responseMessage = PtrToString(message);
                    ...
                    if (referral != IntPtr.Zero) {
                        ...
                        while (singleReferral != null) {
                            string s = PtrToString((IntPtr)singleReferral);
                            referralList.Add(s);
        ...
        internal SearchResultEntry ConstructEntry(IntPtr entryMessage) {
            ...
            try {
                ...
                if (dn != IntPtr.Zero) {
                    entryDn = PtrToString(dn);//Marshal.PtrToStringUni(dn);
                    Wldap32.ldap_memfree(dn);
                    dn = IntPtr.Zero;
                }
        ...
        private DirectoryControl ConstructControl(IntPtr controlPtr) {
            ...
            string controlType = PtrToString(control.ldctl_oid);
